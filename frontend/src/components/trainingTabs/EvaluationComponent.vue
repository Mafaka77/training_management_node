<template>
    <div class="mx-auto">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Evaluation</h1>
            </div>


            <div v-if="!store.traineeEvaluationIsLoading && store.traineeEvaluations?.length > 0"
        ...
        <div v-if="store.traineeEvaluationIsLoading" class="space-y-4">
        ...
        <div v-else>
            ...
            <div v-if="store.traineeEvaluations?.length > 0" class="space-y-4">
                <div v-for="evaluation in store.traineeEvaluations" :key="evaluation.id || evaluation._id"
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <h3 class="font-bold text-slate-800">{{ evaluation.trainee?.full_name || 'Unknown Trainee'
                            }}
                            </h3>
                    ...
                    <div class="flex items-center gap-3">
                        <router-link
                            :to="{ name: 'training.evaluation', params: { id: trainingId, evaluationId: evaluation._id } }"
                                    ...
                                    <div class="bg-white rounded-[28px] w-full max-w-sm p-6 shadow-2xl scale-in">
                                        ...
                                        <div class="flex gap-3">
                                            ...
                                            <button @click="confirmDelete(certificate)"
                                                :disabled="store.traineeEvaluationIsLoading"
                                                ...
                                                <svg v-if="store.traineeEvaluationIsLoading"
            ...
            <div v-else
                ...
                <h3 class="text-lg font-bold text-slate-700">No Evaluation Found</h3>
                <p class="text-slate-400 text-sm max-w-xs text-center mt-2">
                    Evaluation data have not been submitted by the trainees for this training program yet.
                </p>
            </div>
        </div>
    </div>
...
// Core fetch function
const loadData = () => {
    store.fetchTraineeEvaluation(trainingId);
};
...
const confirmDelete = async (certificate) => {
    const response = await store.deleteReleaseOrder(trainingId);
    if (response.success) {
