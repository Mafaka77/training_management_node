<template>
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
    <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-400">
      <router-link to="/trainer/dashboard" class="hover:text-blue-500 transition-colors">Dashboard</router-link>
      <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
        <path d="M9 5l7 7-7 7" />
      </svg>
      <router-link to="/trainer/trainings" class="hover:text-blue-500 transition-colors">My Trainings</router-link>
      <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
        <path d="M9 5l7 7-7 7" />
      </svg>
    ...
    <div class="flex items-center justify-between">
      <div class="space-y-1">
        <div class="flex items-center gap-3">
          <h2 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">MATERIALS</h2>
        </div>
        <div class="flex items-center gap-3">
        <h3 class="text-md font-bold text-zinc-900 dark:text-zinc-100">{{route.params.name}}</h3>
      </div>
        <p class="text-sm text-zinc-500 ">Upload PDFs, slides, or documents</p>
      </div>
      <button
    ...
    <div v-if="isMaterialLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="i in 6" :key="i" class="p-4 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-white/5 rounded-2xl flex items-center gap-4 animate-pulse">
        <div class="w-12 h-12 rounded-xl bg-zinc-100 dark:bg-white/5"></div>

        <div class="flex-1 space-y-3">
          <div class="h-4 bg-zinc-100 dark:bg-white/5 rounded w-3/4"></div>

          <div class="space-y-2">
            <div class="flex items-center gap-2">
              <div class="h-3 bg-zinc-100 dark:bg-white/5 rounded w-10"></div>
              <div class="h-3 bg-zinc-100 dark:bg-white/5 rounded w-12"></div>
            </div>
            <div class="flex items-center gap-2">
              <div class="w-4 h-4 rounded-full bg-zinc-100 dark:bg-white/5"></div>
              <div class="h-3 bg-zinc-100 dark:bg-white/5 rounded w-24"></div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div v-else-if="materials.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div
...
const {materials,isMaterialLoading}=storeToRefs(store);
...
const handleUpload = async () => {
  ...
  formData.append('programId', route.params.id);

  try {
    const response=await store.submitMaterial(formData);
    if(response.success){
      alert.success(response.message);
      await store.fetchMaterials(route.params.id);
      uploadForm.title = '';
