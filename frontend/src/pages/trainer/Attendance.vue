  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-4">
        <router-link to="/trainer/trainings" class="p-2 rounded-xl bg-zinc-100 dark:bg-white/5 text-zinc-500 hover:text-blue-500 transition-colors">
        ...
        <div>
          ...
          <p class="text-xs text-zinc-500 uppercase tracking-widest font-black">Mark Attendance</p>
        </div>
      </div>

      <button :disabled="isAttendanceLoading" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-lg shadow-blue-500/20 transition-all active:scale-95 disabled:opacity-50">
        Export
      </button>
    </div>
...
import { CheckCircleIcon, XCircleIcon, ChevronLeftIcon,Loader2Icon } from "lucide-vue-next";
...
const {trainees,sessionTopic,isAttendanceLoading}=storeToRefs(store);
...
onMounted(()=>{
  store.fetchTrainees(route.params.id);
});
</script>