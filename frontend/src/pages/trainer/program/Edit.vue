    ...
    <nav class="flex p-3 bg-white/5 border border-white/10 rounded-xl mb-6" aria-label="Breadcrumb">
      <ol class="inline-flex items-center space-x-1 md:space-x-2">
        <li class="inline-flex items-center">
          <router-link to="/trainer/programs" class="text-sm font-medium text-zinc-400 hover:text-blue-500 flex items-center">
...
async function fetchProgramData(id) {
  loading.value = true;
  try {
    const data = await directorStore.fetchTraining(id);
