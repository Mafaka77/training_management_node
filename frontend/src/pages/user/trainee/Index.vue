  ...
  <div class="space-y-6">
    <div class="flex flex-col gap-6 mb-8">
      ...
      <div class="relative max-w-md">
        ...
        <input v-model="searchQuery" type="text" placeholder="Search by name, email, mobile..." class="block w-full pl-10 pr-4 py-2.5 text-sm rounded-xl border transition-all
    ...
    <div v-else class="grid grid-cols-1 gap-4">
      <div v-for="trainee in trainees" :key="trainee._id" class="group relative flex flex-col md:flex-row md:items-center gap-5 p-6 rounded-2xl border transition-all duration-300
               bg-white dark:bg-zinc-900/50
               border-zinc-200 dark:border-white/10
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-3 mb-1">
            ...
            <span
              class="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-blue-100 dark:bg-blue-900/30 text-blue-600 border border-blue-200 dark:border-blue-800">
              Trainee
            </span>
            <span v-if="trainee.is_blacklisted"
              class="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-red-100 dark:bg-red-900/30 text-red-600 border border-red-200 dark:border-red-800">
              Blacklisted
            </span>
          </div>
        ...
        <div class="flex items-center gap-2 pt-4 md:pt-0 border-t md:border-t-0 border-zinc-100 dark:border-white/5">
          <router-link :to="{ name: 'trainee.edit', params: { id: trainee._id } }" title="Edit"
          ...
          <button @click="openDeleteModal(trainee)" title="Delete"
    ...
    <BaseModal :show="isDeleteModalOpen" confirmVariant="danger" @close="isDeleteModalOpen = false"
      ...
      <template #content>
        Are you sure you want to remove <span class="font-bold text-zinc-900 dark:text-zinc-100">{{
          traineeToDelete?.full_name }}</span>?
