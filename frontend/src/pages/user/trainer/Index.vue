  ...
  <div class="max-w-[90rem] mx-auto pb-10 space-y-6">
      ...
      <div>
        <div class="flex items-center gap-3 mb-1">
          ...
          <span
            ...
            {{ trainersTotal || 0 }} Records
          </span>
        </div>
        <p class="text-xs text-zinc-500 dark:text-zinc-400">
          Manage certified resource persons, master instructors, and faculty across modules.
        </p>
      </div>

      <div class="flex items-center gap-2.5">
        <router-link to="/admin/trainer/create"
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-2 text-xs text-zinc-500">
        <span>Displaying page <strong class="text-zinc-900 dark:text-zinc-100">{{ trainerPage }}</strong> of <strong
            class="text-zinc-900 dark:text-zinc-100">{{ trainerTotalPages }}</strong></span>
      </div>
    </div>
    ...
    <div
            ...
            <tr>
              <td colspan="5" class="px-6 py-20 text-center">
                <div class="flex flex-col items-center justify-center max-w-sm mx-auto">
                  ...
                  <h3 class="text-sm font-bold text-zinc-900 dark:text-zinc-100">No Trainers Found</h3>
                  <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1 leading-relaxed">
                    {{ searchQuery ? `No trainer records matched your search query "${searchQuery}".` : 'There are currently no registered trainers in the system.' }}
                  </p>
                  <button v-if="searchQuery" @click="searchQuery = ''"
            ...
            <tr v-for="trainer in filteredTrainers" :key="trainer._id"
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="space-y-0.5">
                    <div class="flex items-center gap-1.5">
                      <span class="font-bold text-zinc-900 dark:text-zinc-100 text-xs">{{ trainer.full_name }}</span>
                    </div>
                    <div>
                      <span
              ...
              <td class="px-5 py-3.5">
                <div class="space-y-1">
                  ...
                  <div v-if="trainer.department"
                    class="flex items-center gap-1.5 text-[11px] text-zinc-600 dark:text-zinc-400 font-medium">
                    <svg class="w-3.5 h-3.5 text-zinc-400 shrink-0" fill="none" viewBox="0 0 24 24"
                      stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <!-- District Column -->
              <td class="px-5 py-3.5">
                <div v-if="trainer.district?.name || trainer.district"
                  class="flex items-center gap-1 text-[11px] text-zinc-600 dark:text-zinc-400 font-medium">
                  ...
                  <span>{{ trainer.district?.name || trainer.district }}</span>
                </div>
      ...
      <div
        ...
        <span class="text-xs text-zinc-500 font-medium">
          Showing {{ filteredTrainers.length }} of {{ trainersTotal || 0 }} trainers
        </span>

        <div v-if="trainerTotalPages > 1" class="flex items-center gap-1">
          <!-- Previous Button -->
          <button @click="goToPage(trainerPage - 1)" :disabled="trainerPage <= 1 || isLoading"
          ...
          <!-- Next Button -->
          <button @click="goToPage(trainerPage + 1)" :disabled="trainerPage >= trainerTotalPages || isLoading"
            class="p-1.5 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 disabled:opacity-30 transition-colors text-zinc-500 cursor-pointer disabled:cursor-not-allowed">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <BaseModal :show="isDeleteModalOpen" confirmVariant="danger" :confirmLoading="isDeleting"
      ...
      <template #content>
        <p class="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">
          Are you sure you want to permanently delete <strong class="text-zinc-900 dark:text-zinc-100 font-bold">{{
            trainerToDelete?.full_name }}</strong>? This action will remove their assigned teaching profile.
        </p>
      </template>
...
const { trainers, isLoading, trainerPage, trainerTotalPages, trainersTotal } = storeToRefs(store);
...
const visiblePageNumbers = computed(() => {
  const total = trainerTotalPages.value || 1;
  const current = trainerPage.value || 1;
  const delta = 2;
  const range = [];
...
const goToPage = (p) => {
  if (p >= 1 && p <= trainerTotalPages.value) {
    store.fetchTrainers(p, searchQuery.value);
  }
};

// Debounced search
const doSearch = debounce((val) => {
  store.fetchTrainers(1, val);
}, 400);
...
const confirmDelete = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      await store.fetchTrainers(trainerPage.value, searchQuery.value);
      isDeleteModalOpen.value = false;
    } else {
      alert.error(response.message || 'Failed to delete trainer');
    }
  } catch (error) {
    alert.error('Failed to delete trainer');
  } finally {
