    ...
    <div class="flex flex-col gap-6 mb-4">
      ...
      <div class="space-y-4">
        ...
        <div class="relative max-w-md">
          ...
          <input v-model="search" type="text" placeholder="Search programs by name..." class="block w-full pl-10 pr-4 py-2.5 text-sm rounded-xl border transition-all
    ...
    <div class="relative min-h-[400px]">
      ...
      <div v-if="isLoading" class="grid grid-cols-1 gap-4">
        <div v-for="i in 3" :key="i"
          class="animate-pulse flex flex-col md:flex-row gap-6 p-5 rounded-2xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-900/50">
          <div class="w-full md:w-32 h-32 bg-zinc-200 dark:bg-zinc-800 rounded-2xl"></div>
          <div class="flex-1 space-y-4 py-1">
            <div class="h-4 bg-zinc-200 dark:bg-zinc-800 rounded w-3/4"></div>
            <div class="space-y-2">
              <div class="h-3 bg-zinc-200 dark:bg-zinc-800 rounded"></div>
      ...
      <div v-else class="grid grid-cols-1 gap-4">
        <div v-for="program in programs" :key="program._id" class="group relative flex flex-col md:flex-row md:items-center gap-6 p-5 rounded-2xl border transition-all duration-300
                 bg-white dark:bg-zinc-900/50
                 border-zinc-200 dark:border-white/10
          ...
          <div class="relative shrink-0">
            ...
            <div
              :class="['absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 border-white dark:border-zinc-900 shadow-sm animate-pulse', getStatusStyle(program.t_status).color]">
            </div>
          </div>
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-2">
              ...
              <span
                :class="['px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border whitespace-nowrap', getStatusStyle(program.t_status).badge]">
                {{ program.t_status }}
              </span>
            </div>
            ...
            <div
              class="mt-4 pt-4 border-t border-zinc-100 dark:border-white/5 flex flex-wrap items-center justify-between gap-4">
              <router-link :to="{ name: 'training.session', params: { id: program._id } }" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold
