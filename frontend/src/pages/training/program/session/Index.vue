<template>
  <div class="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      <ol v-else class="inline-flex items-center space-x-1 md:space-x-2">
        <li class="inline-flex items-center">
          <router-link to="/admin/training/program"
            class="text-sm font-medium text-zinc-400 hover:text-blue-500 flex items-center">
    ...
    <section
      class="relative overflow-hidden rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 shadow-sm">
      <div v-if="isLoading" class="flex flex-col md:flex-row items-center gap-6 p-6 animate-pulse">
        <div class="w-full md:w-48 h-32 bg-zinc-200 dark:bg-zinc-800 rounded-2xl"></div>
        <div class="flex-1 space-y-3">
          ...
          <div class="h-8 w-40 bg-zinc-100 dark:bg-zinc-800 rounded-lg"></div>
        </div>
        <div class="flex gap-4 p-4 bg-zinc-50 dark:bg-white/5 rounded-2xl">
          <div class="h-10 w-16 bg-zinc-200 dark:bg-zinc-800 rounded-lg"></div>
          <div class="h-10 w-16 bg-zinc-200 dark:bg-zinc-800 rounded-lg"></div>
        </div>
      </div>

      <div v-else class="flex flex-col md:flex-row items-center gap-6 p-6">
        <div class="relative w-full md:w-48 h-32 shrink-0">
          <img :src="getImageUrl(form.t_banner)"
            class="w-full h-full object-cover rounded-2xl border border-zinc-100 dark:border-white/5 shadow-inner" />
        </div>
        <div class="flex-1 space-y-2">
          <div class="flex items-center gap-3">
            <h1 class="text-xl font-extrabold text-zinc-900 dark:text-zinc-100 tracking-tight">{{ form.t_name }}</h1>
            <span
              class="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-blue-50 dark:bg-blue-500/10 text-blue-600 border border-blue-100 dark:border-blue-500/20 uppercase tracking-wider">{{
                form.t_status }}</span>
          </div>
          <div class="flex items-center gap-4 mt-2">
            <span
              class="flex items-center gap-1.5 text-xs font-semibold text-zinc-600 dark:text-zinc-400 bg-zinc-100 dark:bg-white/5 px-3 py-1.5 rounded-lg border border-zinc-200 dark:border-white/10">
    ...
    <div
      class="flex items-center gap-1 p-1 bg-zinc-100 dark:bg-white/5 rounded-2xl w-fit border border-zinc-200 dark:border-white/10">
      <template v-if="isLoading">
        <div v-for="i in 4" :key="i" class="h-10 w-32 bg-zinc-200 dark:bg-zinc-800 rounded-xl animate-pulse"></div>
      </template>
      <template v-else>
        <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" :class="[
          'flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-xl transition-all duration-200',
          activeTab === tab.id ? 'bg-white dark:bg-zinc-800 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200'
        ]">
    ...
    <div>
      <div v-if="activeTab === 'schedule'" class="space-y-6">
        <div v-if="isSessionLoading" class="space-y-6 animate-pulse">
          <div class="flex justify-between items-center">
            <div class="space-y-2">
              ...
              <div class="h-4 w-64 bg-zinc-100 dark:bg-zinc-800 rounded"></div>
            </div>
            <div class="h-10 w-32 bg-zinc-200 dark:bg-zinc-800 rounded-xl"></div>
          </div>
          <div v-for="i in 3" :key="i"
            class="h-20 w-full bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 rounded-2xl">
          </div>
        </div>
        ...
        <template v-else>
          ...
          <div v-if="Object.keys(groupedSessions).length === 0"
            class="py-16 text-center bg-zinc-50 dark:bg-white/5 rounded-3xl border-2 border-dashed border-zinc-200 dark:border-white/10">
            ...
            <div v-for="(daySessions, date) in groupedSessions" :key="date"
              ...
              <button @click="toggleDate(date)"
                ...
                <div class="flex items-center gap-4">
                  <div
                    class="px-3 py-1 rounded-lg bg-zinc-100 dark:bg-white/10 text-xs font-bold font-mono text-zinc-600 dark:text-zinc-300">
                    {{ formatDate(date) }}
                  </div>
                  ...
                  <span
                    class="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-500/20 text-blue-600 text-[9px] font-black uppercase tracking-tighter">
              ...
              <div v-show="expandedDates.has(date)" class="border-t border-zinc-100 dark:border-white/5">
                <table class="w-full text-left">
                  <thead
                    class="bg-zinc-50 dark:bg-white/[0.02] text-[10px] font-black uppercase text-zinc-400 border-b border-zinc-100 dark:border-white/5">
                    <tr>
                  ...
                  <tbody class="divide-y divide-zinc-100 dark:divide-white/5">
                    <tr v-for="session in daySessions" :key="session._id"
                      class="group hover:bg-blue-50/30 dark:hover:bg-blue-500/[0.02] transition-colors">
                      <td class="px-6 py-4">
                      ...
                      <td class="px-6 py-4">
                        <span class="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-tight">{{
                          session.tc_topic }}</span>
                      </td>
                      <td class="px-6 py-4">
                        <div v-if="session.trainer" class="flex items-center gap-2">
                          <img :src="getImageUrl(session.trainer?.image, session.trainer?.full_name)"
                            class="w-6 h-6 rounded-full object-cover border border-zinc-200 dark:border-white/10" />
      ...
      <div v-if="activeTab === 'attendance'" class="space-y-6">
        <div v-if="form.t_status === 'Upcoming' || form.t_status === 'Draft'"
          ...
          <p class="text-xs text-zinc-500 mt-1 max-w-[240px] text-center leading-relaxed"> Attendance tracking becomes
            available once the training status moves to <span class="text-blue-500 font-bold">Ongoing</span>. </p>
        </div>
        <div v-else>
      ...
      <div v-if="activeTab === 'release-order'" class="space-y-6">
        ...
        <div v-if="form.t_status === 'Upcoming' || form.t_status === 'Draft' || form.t_status === 'Ongoing'"
          ...
          <h2 class="text-sm font-black uppercase tracking-widest text-zinc-900 dark:text-zinc-100">Registry Locked</h2>
          <p class="text-xs text-zinc-500 mt-1 max-w-[240px] text-center leading-relaxed"> Release Order becomes
      ...
      <Transition enter-active-class="duration-300 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100"
        ...
        <div v-if="showModal"
          ...
          <div
            class="bg-white dark:bg-zinc-900 w-full max-w-lg rounded-3xl shadow-2xl border border-zinc-200 dark:border-white/10 overflow-hidden">
            <div
              class="px-6 py-4 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between bg-zinc-50/50 dark:bg-white/2">
            ...
            <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                  ...
                  <input v-model="sessionForm.tc_date" type="date" required
                    class="w-full px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-zinc-800 text-sm outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" />
                </div>
              ...
              <div class="flex items-center justify-end gap-3 pt-4 border-t border-zinc-100 dark:border-white/5">
                <button type="button" @click="closeModal"
                  class="px-5 py-2.5 text-sm font-bold text-zinc-500 hover:bg-zinc-100 dark:hover:bg-white/5 rounded-xl transition-all">Cancel</button>
                <button type="submit" :disabled="isSubmitting"
                  class="px-6 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl shadow-lg shadow-blue-500/30 hover:bg-blue-700 transition-all disabled:opacity-50 flex items-center gap-2">
