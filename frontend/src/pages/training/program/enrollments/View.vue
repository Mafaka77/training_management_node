                ...
                <div
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <p class="text-xs text-zinc-500 mt-0.5">
                                Detailed information and history for this application.
                            </p>
                        </div>
                        <button @click="$emit('close')"
                            class="p-2 rounded-md hover:bg-zinc-200 dark:hover:bg-white/10 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors cursor-pointer">
                    ...
                    <div class="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar bg-white dark:bg-zinc-950">
                        <div v-if="loading" class="py-24 flex flex-col items-center justify-center gap-3">
                            ...
                            <span class="text-xs font-bold text-zinc-500 uppercase tracking-widest">Loading
                                Details...</span>
                        </div>

                        <div v-else-if="enrollment" class="max-w-4xl mx-auto space-y-8">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-1">
                                        <h4 class="text-2xl font-black text-zinc-900 dark:text-white truncate">
                                            {{ enrollment.training_program?.t_name || 'Training Program' }}
                                        </h4>
                                    ...
                                    <p class="text-sm text-zinc-600 dark:text-zinc-400 font-medium mt-1 truncate">
                                        {{ enrollment.training_program?.t_description || 'No Description Provided' }}
                                    </p>
                            ...
                            <div class="space-y-4">
                                <div class="flex items-center justify-between px-1">
                                    <h5
                                        class="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-widest flex items-center gap-2">
                                ...
                                <div
                                    class="p-4 bg-zinc-50/80 dark:bg-zinc-900/60 border border-zinc-200/80 dark:border-white/10 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div class="flex items-center gap-3.5">
                                        <div
                                            class="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-600 to-teal-700 text-white font-black text-lg flex items-center justify-center shadow-md shrink-0">
                                        ...
                                        <div>
                                            <h4
                                                ...
                                                <span v-if="enrollment.user?.gender"
                                                    class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-zinc-200/80 dark:bg-white/10 text-zinc-700 dark:text-zinc-300">
                                                    {{ enrollment.user?.gender }}
                                                </span>
                                            </h4>
                                            <p class="text-xs text-zinc-500 dark:text-zinc-400 font-medium">
                                                {{ enrollment.user?.designation || 'No Designation' }} • {{
                                                    enrollment.user?.department || 'No Office' }}
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div
                                    class="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-white/10 rounded-xl overflow-hidden shadow-xs transition-all">
                                    <button @click="tiles.personal = !tiles.personal" type="button"
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <span
                                                    class="text-xs font-bold text-zinc-900 dark:text-zinc-100">Personal
                                                    & Contact Details</span>
                                                <span class="text-[11px] text-zinc-400 ml-2 hidden sm:inline">Phone,
                                                    Email, DOB, District, Category</span>
                                            </div>
                                        </div>
                                        <svg class="w-4 h-4 text-zinc-400 transition-transform duration-200"
                                            :class="tiles.personal ? 'rotate-180 text-emerald-600' : ''" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </button>

                                    <div v-show="tiles.personal"
                                        class="p-5 border-t border-zinc-100 dark:border-white/5 bg-white dark:bg-zinc-900">
                                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                                            <div>
                                                ...
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.full_name || 'N/A' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                            ...
                                            <div>
                                                <dt
                                                    ...
                                                    Date of Birth</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    formatDate(enrollment.user?.dob) }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    District</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    getDistrictName(enrollment.user?.district) || 'N/A' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Category</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.category || 'General' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                ...
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.qualification || 'Not Specified' }}</dd>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div
                                    ...
                                    <button @click="tiles.employment = !tiles.employment" type="button"
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <span class="text-xs font-bold text-zinc-900 dark:text-zinc-100">Service
                                                    & Employment Details</span>
                                                <span
                                                    class="text-[11px] text-zinc-400 ml-2 hidden sm:inline">Designation,
                                                    Department, Service Dates, Confirmation</span>
                                            </div>
                                        </div>
                                        <svg class="w-4 h-4 text-zinc-400 transition-transform duration-200"
                                            :class="tiles.employment ? 'rotate-180 text-indigo-600' : ''" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </button>

                                    <div v-show="tiles.employment"
                                        class="p-5 border-t border-zinc-100 dark:border-white/5 bg-white dark:bg-zinc-900">
                                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                                            <div>
                                                <dt
                                                    ...
                                                    Office</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.department || 'N/A' }}</dd>
                                            </div>
                                            <div v-if="enrollment.user?.departmentParent">
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Department</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    getDepartmentParentName(enrollment.user?.departmentParent) || 'N/A'
                                                }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    ...
                                                    Designation</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.designation || 'N/A' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Group / Cadre Group</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    getGroupName(enrollment.user?.group) || 'N/A' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Service Type</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.service || 'State' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Service Cadre</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.service_cadre || 'Not Specified' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Govt Employee Status</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">
                                                    <span
                                                        :class="enrollment.user?.is_govt_employee !== false ? 'text-emerald-600' : 'text-zinc-500'">
                                                        {{ enrollment.user?.is_govt_employee !== false ? 'Yes' +
                                                            '(Government)' : 'No / Non-Govt' }}
                                                    </span>
                                                </dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Mode of Recruitment</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.recruitment || 'Direct' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Confirmation Status</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    enrollment.user?.confirmation || 'Confirmed' }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Date of Entry into Service</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    formatDate(enrollment.user?.date_of_entry) }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Date of Entry in Present Grade</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                            ...
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Date of Superannuation (Retirement)</dt>
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">{{
                                                    formatDate(enrollment.user?.date_of_superannuation) }}</dd>
                                            </div>
                                            <div>
                                                <dt
                                                    class="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
                                                    Mandatory Training Status</dt>
                                                <dd class="text-xs font-bold mt-0.5">
                                                    <span
                                                        :class="enrollment.user?.mandatory_completion ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-500'">
                                                        {{ enrollment.user?.mandatory_completion ? '✓ Completed' :
                                                            'Pending Completion' }}
                                                    </span>
                                                </dd>
                                            </div>
                                ...
                                <div
                                    ...
                                    <button @click="tiles.status = !tiles.status" type="button"
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <span
                                                    class="text-xs font-bold text-zinc-900 dark:text-zinc-100">Discipline
                                                    & Blacklist Check</span>
                                                <span
                                                    class="text-[11px] text-zinc-400 ml-2 hidden sm:inline">Eligibility,
                                                    Active Status, Penalty Records</span>
                                            </div>
                                        </div>
                                        <svg class="w-4 h-4 text-zinc-400 transition-transform duration-200"
                                            :class="tiles.status ? 'rotate-180 text-amber-600' : ''" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </button>

                                    <div v-show="tiles.status"
                                        class="p-5 border-t border-zinc-100 dark:border-white/5 bg-white dark:bg-zinc-900">
                                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6">
                                            <div>
                                                <dt
                                                ...
                                                <dd class="text-xs font-bold text-zinc-800 dark:text-zinc-200 mt-0.5">
                                                    <span
                                                        class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[11px] font-bold"
                                                        ...
                                                        <span class="w-1.5 h-1.5 rounded-full"
                                                            :class="enrollment.user?.is_active !== false ? 'bg-emerald-600' : 'bg-rose-600'"></span>
                                            ...
                                            <div>
                                                ...
                                                <dd class="text-xs font-bold mt-0.5">
                                                    <span
                                                        ...
                                                        {{ enrollment.user?.is_blacklisted ? '⚠️ Blacklisted' :
                                                            '✓ Clean'
                                                            + ' Record' }}
                                                    </span>
                                                </dd>
                                            ...
                                            <div v-if="enrollment.user?.is_blacklisted">
                                                ...
                                                <dd class="text-xs font-medium text-rose-600 dark:text-rose-400 mt-0.5">
                                                    {{ enrollment.user?.blacklist_details?.reason || 'No Reason Stated'
                                                    }}</dd>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div>
                                <h5
                                    class="text-xs font-bold text-zinc-900 dark:text-white uppercase tracking-widest mb-3 px-1 flex items-center gap-2">
                                    <svg class="w-4 h-4 text-emerald-600 dark:text-emerald-400" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div v-if="enrollment"
                        ...
                        <button @click="$emit('close')"
                            class="px-5 py-2 text-zinc-600 hover:bg-zinc-200 dark:hover:bg-white/10 dark:text-zinc-400 dark:hover:text-white rounded-md text-sm font-bold transition-colors cursor-pointer">
                            Close
                        </button>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <template v-if="enrollment.status === 'Pending'">
                                <button @click="$emit('update-status', enrollment._id, 'Rejected')"
                                    class="px-5 py-2 text-rose-600 bg-rose-50 hover:bg-rose-100 dark:bg-rose-500/10 dark:hover:bg-rose-500/20 rounded-md text-sm font-bold transition-colors cursor-pointer">
                                    Reject
                                </button>
                                <button @click="$emit('update-status', enrollment._id, 'Approved')"
                                    class="inline-flex items-center gap-2 px-5 py-2 bg-zinc-900 hover:bg-zinc-800 dark:bg-white dark:hover:bg-zinc-200 dark:text-zinc-900 text-white rounded-md text-sm font-bold transition-colors shadow-sm cursor-pointer">
                            ...
                            <template v-else>
                                <button @click="$emit('update-status', enrollment._id, 'Pending')"
                                    class="inline-flex items-center gap-2 px-5 py-2 bg-amber-50 hover:bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:hover:bg-amber-500/20 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 rounded-md text-sm font-bold transition-colors shadow-sm cursor-pointer">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
...
// DepartmentParent name helper
const getDepartmentParentName = (deptParent) => {
    if (!deptParent) return '';
    if (typeof deptParent === 'object') return deptParent.name || '';
    return String(deptParent);
};
