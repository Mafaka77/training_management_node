            ...
            <section
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2">
                        <BaseInput v-model="form.name" label="Category Name" placeholder="e.g. Technical Skills"
                            type="text" />
                    </div>

                    <BaseInput v-model="form.description" label="Category Description"
                        placeholder="Briefly describe what this training category covers..." type="text" />
                </div>
...
const submitForm = async () => {
    ...
    try {
        const response = await store.updateCategory(route.params.id, form);
        if (response.success === false) {
            alert.error(response.message);
        } else {
            alert.success(response.message || "Category updated successfully!");
            form.name = '';
            form.description = '';
            router.back()
...
const fetchCategory = async () => {
    try {
        const res = await store.fetchCategory(route.params.id);
        if (res.success) {
