<template>
  <nav class="flex p-3 bg-white/5 border border-white/10 rounded-xl mb-6" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-2">
      <li class="inline-flex items-center">
        <router-link to="/admin/master/room"
          class="inline-flex items-center text-sm font-medium text-zinc-400 hover:text-blue-500">
  ...
  <form v-else @submit.prevent="handleUpdate" class="space-y-4">
    <div class="bg-white/5 rounded-2xl p-6 space-y-4 border border-white/10">
      ...
      <BaseInput v-model="form.room_name" label="Room Name" placeholder="e.g. Room A101" type="text" />

      <BaseInput v-model.number="form.capacity" label="Room Capacity" placeholder="e.g. 30" type="number" />

      <BaseInput v-model="form.details" label="Room Details" placeholder="Room description..." type="text" />
      ...
      <div class="flex gap-3 pt-2">
        ...
        <button type="button" @click="$router.push('/admin/master/room')"
