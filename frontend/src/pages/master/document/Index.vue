<template>
  <div class="space-y-4">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
      <div class="space-y-1">
        ...
        <p class="text-sm text-zinc-500 dark:text-zinc-400">
          Organize your documents into functional groups
        </p>
      </div>

      <router-link to="/admin/master/documents/create"
    ...
    <div v-if="isDocLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    <div v-else-if="documents.length === 0"
      ...
      <h3 class="text-zinc-900 dark:text-zinc-100 font-medium">No documents found</h3>
      <p class="text-zinc-500 dark:text-zinc-400 text-sm">Add a document to you repos.</p>
    </div>

    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="cat in documents" :key="cat._id" class="group relative flex items-center gap-4 p-4 rounded-2xl border transition-all duration-300
        ...
        <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <router-link :to="{ name: 'master.documents.edit', params: { id: cat._id } }"
  ...
  <BaseModal :show="isDeleteModalOpen" confirmVariant="danger" @close="isDeleteModalOpen = false"
    @confirm="confirmDelete">
    <template #icon>
      <div
        class="w-12 h-12 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-500 rounded-full flex items-center justify-center mb-4">
    ...
    <template #content>
      Are you sure you want to delete <span class="font-bold text-zinc-900 dark:text-zinc-100">{{
        roomToDelete?.room_name }}</span>?
...
const confirmDelete = async () => {
  ...
  const response = await store.deleteDocument(docToDelete.value._id);
  if (response.success) {
...
const { documents, isDocLoading } = storeToRefs(store);
